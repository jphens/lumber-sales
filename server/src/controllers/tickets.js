  ...
  createTicket(req, res) {
    try {
      ...
      if (!id || !customerName || !date || !items || !Array.isArray(items)) {
        ...
        return res
          .status(400)
          .json({ error: "Missing required ticket information" });
      }
      ...
      for (const item of items) {
        if (
          ...
          return res.status(400).json({
            error:
              "Each item must include quantity, width, thickness, length, and price_per_mbf",
          });
        }
      ...
      const ticket = {
        ...
        ticket_type: ticket_type || "invoice",
        purchase_order: purchase_order || null,
        shipping_attention: shipping_attention || null,
        total_freight: parseFloat(total_freight) || 0,
        status: status || "draft",
